...
//	
//	
//	@RequestMapping(value = "/new", method = RequestMethod.POST) //nhận request thì client vào
//	@ResponseBody //nhận dữ liệu json được truyền từ client vào server
//	public NewDTO createNew(@RequestBody NewDTO model) {  //trả kết quả json từ server về client
//		return model;
//	}
...
public class NewAPI {
	...
	public NewOutput showNew(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "limit", required = false) Integer limit) {
		...
		if( page != null && limit != null) {
			...
			result.setTotalItem((int) Math.ceil((double) (newService.totalItem())/ limit));
		} else {
